...
export function MultiSelectFilter({
  ...
  return (
    <div ref={rootRef} className={cn("group relative", className ?? "w-[160px]")}>
      <button
        type="button"
        ...
        {display === "tags" && value.length ? (
          <span className="flex min-w-0 items-center gap-1">
            {visibleTags.map((item) => (
              <span
                key={item}
                className="inline-flex max-w-[104px] items-center rounded-sm bg-bg-page px-2 py-0.5 text-small text-text-primary"
              >
      ...
      {open ? (
        <div className="absolute left-0 top-[calc(100%+4px)] z-30 max-h-[260px] w-full min-w-[220px] overflow-auto rounded-sm border border-border bg-white p-1 shadow-md">
